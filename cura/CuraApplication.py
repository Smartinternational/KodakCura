...
try:
    ...
    CuraVersion = "3.6.1"  # [CodeStyle: Reflecting imported value]
    CuraBuildType = ""
    CuraDebugMode = False
    CuraSDKVersion = ""
...
class CuraApplication(QtApplication):
    ...
    def startSplashWindowPhase(self) -> None:
        ...
        preferences.addPreference("cura/active_mode", 1)
    ...
    def runWithGUI(self):
        ...
        if diagonal < 1: #No printer added yet. Set a default camera distance for normal-sized printers.
            diagonal = 375
        camera.setPosition(Vector(-120, 250, 700) * diagonal / 375)
        camera.setPerspective(True)
        camera.lookAt(Vector(-40, 0, 0))
        controller.getScene().setActiveCamera("3d")

        # Initialize camera tool
        camera_tool = controller.getTool("CameraTool")
        camera_tool.setOrigin(Vector(-40, 100, 0))
