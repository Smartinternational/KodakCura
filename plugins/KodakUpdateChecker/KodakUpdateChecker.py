...
from .KodakUpdateCheckerJob import KodakUpdateCheckerJob
...
class KodakUpdateChecker(Extension):
    url = "http://www.smart3d.tech/download_files/setup/latest.json"
    ...
    def checkNewVersion(self, silent = False, display_same_version = True):
        self._download_url = None
        job = KodakUpdateCheckerJob(silent, display_same_version, self.url, self._onActionTriggered, self._onSetDownloadUrl)
        job.start()
